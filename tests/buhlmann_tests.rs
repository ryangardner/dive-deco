use dive_deco::{
    BreathingSource, BuhlmannConfig, BuhlmannModel, CeilingType, Deco, DecoModel, Depth, GasMix,
...
fn test_example_ceiling_feet() {
    ...
    assert_eq!(model.ceiling().as_feet(), 41.06408);
    assert_eq!(model.ceiling().as_meters(), 12.516331);
}
...
fn test_deco_cal_tts_low_surface_atm() {
    ...
    let deco_runtime = deco.calc(model, &[air]).expect("Deco calc failed");
