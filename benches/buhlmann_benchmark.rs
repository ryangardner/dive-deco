    ...
    c.bench_function("Buhlmann NDL", |b| {
        b.iter(|| {
            ...
            model.record(
                ...
                Time::from_seconds(5.),
                &BreathingSource::OpenCircuit(GasMix::air()),
            );
            model.ndl();
        })
    });
...
pub fn buhlmann_deco_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("Buhlmann deco", |b| {
        b.iter(|| model.deco(&[air, ean_50], false))
    });
}
...
pub fn buhlmann_deco_adaptive_recalc(c: &mut Criterion) {
    ...
    c.bench_function("Record and deco", |b| {
        b.iter(|| {
            ...
            model.deco(&available_gasses, false).unwrap();
            model.record(Depth::from_meters(40.), Time::from_seconds(1.), &air);
            model.record(Depth::from_meters(40.), Time::from_seconds(1.), &air);
            model.deco(&available_gasses, false).unwrap();
        });
...
pub fn buhlmann_full(c: &mut Criterion) {
    ...
    let air = BreathingSource::OpenCircuit(GasMix::air());
    let ean50 = BreathingSource::OpenCircuit(GasMix::try_new(0.50, 0.).unwrap());
    let o2 = BreathingSource::OpenCircuit(GasMix::try_new(1., 0.).unwrap());
